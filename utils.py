...
def get_test_image(paths, height=None, width=None, flag=False, oc=2):
    ...
    for path in paths:
        ...
        if height is not None and width is not None:
            ...
            image = np.array(Image.fromarray(image).resize((width, height), Image.BICUBIC))
        ...
        if h > base_size or w > base_size:
            ...
            # images = get_img_parts(image, h, w)
            images, crop_number = crop_image(image, base_size, base_size, oc)
            c = images.shape[0]
            images = np.reshape(images, [crop_number, 1, base_size, base_size])
...
def save_image_test(img_fusion, output_path=1):
